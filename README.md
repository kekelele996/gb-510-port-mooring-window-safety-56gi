...
- 风浪窗口带生效时间与失效时间，安全许可按「区域 + 窗口编码」关联窗口：提交时固化当时的窗口版本与有效期；复核放行前重新读取窗口，窗口换版、受限或过期一律拒绝放行并提示重新提交。
- 窗口变为受限或过期后，同区域通过窗口编码关联的待处理许可立即级联失效（写入失效原因）；许可读取时还会按冻结有效期做惰性过期。受限/过期许可可由 operator「重新提交」回到双人确认流程。
- `ClearancePanel` 在风浪窗口和许可页共用，呈现关联窗口、冻结版本/有效期与失效提示；许可审计显式保存窗口版本、操作者和请求 ID。
