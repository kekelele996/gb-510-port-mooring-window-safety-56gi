...
type WeatherWindowRepository interface {
	...
	FindByFacilityCode(context.Context, string, string) (model.WeatherWindow, error)
...
func (r *weatherWindowRepository) FindByFacilityCode(ctx context.Context, facility, code string) (model.WeatherWindow, error) {
	return r.store.FirstWhere(ctx, "facility = ? AND code = ?", facility, code)
}
