...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	safetyClearanceService := service.NewSafetyClearanceService(safetyClearanceRepository, weatherWindowRepository, securityService, logger)
	weatherWindowService := service.NewWeatherWindowService(weatherWindowRepository, safetyClearanceService, securityService)
